...
export default function TasksList({ currentUser, tasks }: TasksListProps){
                ...
                <div className="flex justify-between">                   
                  <div className="flex">
                    ...
                    <button className='text-3xl' onClick={() => toggleDetails(x)}>
                      {detailsVisible[x] ? <BsChevronUp className="text-customBrown h-5" /> : <BsChevronDown className="text-customBrown h-5" /> }
                    </button>
                    </div>  
                    <div>
                        <button className="mx-4 py-2 px-2 rounded-full bg-custombgpink4"
                            onClick={() => updateScreen(task)}>
                            <FiEdit3 className="text-customBrown" />  
                        </button>
                        <button className="mx-4 py-2 px-2 rounded-full bg-red-300"> 
                            <FaTrash className="text-customBrown"
                  ...
                  {subScreen === 'update' && (
                    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-10">
                      <div className="bg-custombgpink4 w-5/12 p-5 mb-80 rounded-2xl">
                        <h2 className="text-2xl font-semibold text-customBrown mb-4">Editar Tarefa</h2>
                        <form className="w-full px-2" onSubmit={updateTask}>
                          <div className="flex justify-center items-center bg-white rounded-t-xl px-2">
                            <p className="font-semibold text-customBrown">Nome:</p>
                            <input
                              ref={titleInputRef}
                              type="text"
                              defaultValue={editTask?.title}
                              placeholder="Nome da tarefa"
                              className="w-full rounded-t-xl text-base text-customBrown focus:text-customBrown focus:outline-none py-2 px-3"
                            />
                          </div>
                          
                          <div className="flex justify-center items-center bg-white px-2">
                            <p className="font-semibold text-customBrown">Descrição:</p>
                            <input
                              ref={descInputRef}
                              type="text"
                              defaultValue={editTask?.description}
                              placeholder="Descrição"
                              className="w-full text-base focus:outline-none focus:text-customBrown text-customBrown py-2 px-3"
                            />
                          </div>
                          
                          <div className="flex justify-center items-center bg-white rounded-b-xl px-2">
                            <p className="font-semibold text-customBrown">Obs:</p>
                            <input
                              ref={obsInputRef}
                              type="text"
                              defaultValue={editTask?.observation}
                              placeholder="Obs"
                              className="w-full rounded-b-xl focus:outline-none focus:text-customBrown text-customBrown py-2 px-3"
                            />
                          </div>
